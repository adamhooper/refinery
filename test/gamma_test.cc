...
typedef refinery::GammaCurve<refinery::RGBImage::ValueType> TestGammaCurve;

std::auto_ptr<TestGammaCurve> createGammaCurveFromImage() {
  ...
  refinery::RGBImage rgbImage(cameraData, 4, 3);
  ...
  return std::auto_ptr<TestGammaCurve>(new TestGammaCurve(histogram));
}

class GammaCurveTest : public ::testing::Test {
};

TEST(GammaCurveTest, FromImage) {
  std::auto_ptr<TestGammaCurve> g(createGammaCurveFromImage());

  EXPECT_EQ(0, g->at(0));
  EXPECT_EQ(16986, g->at(0x1000));
  EXPECT_EQ(0xffff, g->at(0xffff));
}

TEST(GammaCurveTest, FromValues) {
  TestGammaCurve g(0.45, 4.5, 0x9000);

  EXPECT_EQ(0, g.at(0));
  EXPECT_EQ(20295, g.at(0x1000));
  EXPECT_EQ(0xffff, g.at(0xffff));
}
